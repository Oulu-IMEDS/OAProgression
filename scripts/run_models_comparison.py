...
if __name__ == "__main__":
    ...
    tmp = pd.merge(models['lgbm_age_sex_bmi_kl_surg_inj_womac'], models['dl'].drop('Progressor', 1), suffixes=['_lgb', '_dl'], on=('ID', 'Side'))
    ...
    tmp = pd.merge(models['logreg_age_sex_bmi_kl_surg_inj_womac'], models['dl'].drop('Progressor', 1), suffixes=['_logreg', '_dl'], on=('ID', 'Side'))
    ...
    fig, axs = init_auc_pr_plot(dl_preds.Progressor.values)
    for key, color, legend in zip(['lgbm_age_sex_bmi_kl_surg_inj_womac', 'lgbm_stacking_kl'], ['blue', 'red'], ['GBM ref.', 'Stacking w. KL']):        
        tmp_df = models[key]
        compute_and_plot_curves(tmp_df, axs, key=legend, legend=True, color=color)        
    ...
    tmp = pd.merge(models['lgbm_age_sex_bmi_kl_surg_inj_womac'], models['lgbm_stacking_kl'].drop('Progressor', 1), suffixes=['_lgb', '_stacking'], on=('ID', 'Side'))
    ...
    fig, axs = init_auc_pr_plot(dl_preds.Progressor.values)
    for key, color, legend in zip(['lgbm_age_sex_bmi_kl_surg_inj_womac', 'lgbm_stacking_no_kl'], ['blue', 'red'], ['GBM ref.', 'Stacking w/o KL']):        
        tmp_df = models[key]
        compute_and_plot_curves(tmp_df, axs, key=legend, legend=True, color=color)
    ...
    plt.show()
    plt.close(fig)
    tmp = pd.merge(models['lgbm_age_sex_bmi_kl_surg_inj_womac'], models['lgbm_stacking_no_kl'].drop('Progressor', 1), suffixes=['_stacking', '_lgb'], on=('ID', 'Side'))
    ...
    fig, axs = init_auc_pr_plot(dl_preds.Progressor.values)
    for key, color, legend in zip(['logreg_age_sex_bmi_kl_surg_inj_womac', 'lgbm_stacking_no_kl'], ['blue', 'red'], ['LR ref.', 'Stacking w/o KL']):        
        tmp_df = models[key]
        compute_and_plot_curves(tmp_df, axs, key=legend, legend=True, color=color)
    ...
    plt.show()
    plt.close(fig)
    tmp = pd.merge(models['logreg_age_sex_bmi_kl_surg_inj_womac'], models['lgbm_stacking_no_kl'].drop('Progressor', 1), suffixes=['_dl', '_lgb'], on=('ID', 'Side'))
    logp = stats.delong_roc_test(tmp.Progressor, tmp.Prediction_dl, tmp.Prediction_lgb)
    print('P-value (DeLong AUC test):', 10**logp)
